...
const outerPro = ['apiParam', 'data']
// 获取组件配置
function getCmptInfo(cmptId, componentsInfo) {
...
function fillItemData(item) {
    ...
    const { cmptId } = newItem
    // 配置取本地
    let curItem = Vue.prototype.$deepClone(getCmptInfo(cmptId, store.state.user.componentsInfo))
    // item.data = curItem.api ? (store.state.newHomeData && store.state.newHomeData[curItem.module_no] || []) : item.data

    curItem.content = curItem.content || []

    newItem.content = newItem.content || []

    curItem.content && (newItem.content = curItem.content.reduce((a, b) => {
