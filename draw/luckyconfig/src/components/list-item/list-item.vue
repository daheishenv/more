    ...
    >
        ...
        >
            ...
            <c-image
                ...
                mode="scaleToFill"
            />
            <!-- :height="itemObj.imgStyle.height" -->
        </view>
        <view class="gc-list-item-bd">
            ...
            >
                <view class="flex-ajcenter">
                    <view class="flex-1">
                        ...
                        <view
                            v-if="itemData.price_market"
                            class="price-com price-com__market d-din"
                        >{{ itemData.price_market }}</view>
                    </view>
                    <view v-if="!itemData.api_url && itemData.price_market">
                        <c-colors
                            :theme="['t']"
                            :pro="['bgc']"
                ...
                <view class="gc-list-item__desc group-info">
                    <c-colors :theme="['#fa3d23']">
                        <view class="price-com">
                            ...
                            <text
                                v-if="itemObj.type === 'group' && itemData.price_market"
                                class="price-com price-com__market d-din"
                            >{{ priceMarket_ }}</text>
                        </view>
                    </c-colors>
            ...
            <block v-if="itemObj.type === 'seckill'">
                <view class="gc-list-item-num c-ellipsis">{{ itemData.goods_brief }}</view>
            ...
            >
                <c-colors :theme="['s2']">
                    <view class="flex align-center">
                        ...
                        <view
                            v-if="itemData.price_market"
                            class="price-com integral-price__market gc-list-item-num--del"
                        >
                            {{ priceMarket_ }}
                        </view>
                    </view>
            ...
            >
                ...
                <view
                    v-if="itemData.price_market"
                    class="gc-list-item-num gc-list-item-num--del"
                >
                    ¥{{ priceMarket_ }}
                </view>
                <view class="btn-buy-box">
            ...
            >
                ...
                <view
                    v-if="itemData.price_market"
                    class="price-com price-com__market d-din customer_market"
                >
...
export default {
    ...
    computed: {
        ...
        minHeight_() {
            const { imgStyle: {
                height = 0
            }, waterfall } = this.itemObj
            return waterfall ? 0.75 * parseInt(height) : height
        },
        maxHeight_() {
            const { imgStyle: {
                width = 0
            }, waterfall } = this.itemObj
            return waterfall ? 1.5 * parseInt(width) : width
        },
        imgHeight_() {
            const { img_width, img_height } = this.itemData
            const { imgStyle: {
                width, height
            }, waterfall } = this.itemObj
            let newHeight = height
            if (this.mode === 'column' && waterfall) {
                const scale = img_width / img_height
                if (scale > 0.95 && scale < 1.05) newHeight = width
                else newHeight = parseInt(img_height / (img_width / parseInt(width)))
            }
            return this.minHeight_ > newHeight ? this.minHeight_ : newHeight
        },
        imgStyle_() {
            return {
                ...this.itemObj.imgStyle,
                height: this.imgHeight_ + 'rpx',
                maxHeight: this.maxHeight_ + 'rpx'
            }
        }
    },
    methods: {
