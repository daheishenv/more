...
import { deepClone, isEmpty } from '@/common/utils/tools'
...
export const debounce = (function () {
    ...
    return function (func, wait = 500) {
        ...
        // 根据方法内容作为键值,保证唯一性
        let randomText = String(args[0]).replace(/[\r\n]|\s+/g, "");
        let name = 'debounce' + randomText;
...
export function formatUnit(val, unit = 'rpx', deault = 0) {
    return !val ? deault : !isNaN(val) ? (val + unit) : val;
}
...
export const getTime = (type) => {
    ...
    switch (type) {
        case 'year':
            ...
            break;
        case 'mon':
            return `${year}-${getNow(mon)}`
            break;
        case 'date':
            return `${year}-${getNow(mon)}-${getNow(date)}`
            break;
        case 'hours':
            return `${year}-${getNow(mon)}-${getNow(date)} ${getNow(h)}`;
            break;
        case 'minutes':
            return `${year}-${getNow(mon)}-${getNow(date)} ${getNow(h)}:${getNow(m)}`;
            break;
        case 'hoursToMinutes':
            return `${getNow(h)}:${getNow(m)}`;
            break;
        default:
            return `${year}-${getNow(mon)}-${getNow(date)} ${getNow(h)}:${getNow(m)}:${getNow(s)}`;
            break;
    }
}
