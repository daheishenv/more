...
export default {
    ...
    computed: {
        ...
        showPoster_() {
            return String(this.showPoster) === 'false' ? false : true
        },
    },
    data() {
        return {
    ...
    methods: {
        ...
        async shareFc() {
            console.log(this.curSkuInfo, this.priceShowUnit, 'fkfkfkfk')
            ...
            let width = 600,
                imgHeight = this.imgHeight ? this.imgHeight : width,
                height = width + 300
            try {
                    ...
                    drawArray: ({ bgObj, type, bgScale }) => {
                        ...
                        return new Promise((rs, rj) => {
                            let postArr = [
                                ...
                                {
                                    type: 'text',
                                    text: this.curSkuInfo.price_show ? '¥' : '',
                                    size: priceTagSize,
                                ...
                                {
                                    type: 'text',
                                    text: this.curSkuInfo.price_market ? `¥${this.curSkuInfo.price_market}${this.priceShowUnit}` : '',
                                    size: priceTagSize,
                                    color: '#999',
                                    dy: imgHeight + 1.5 * u24 * 3,
                                    textAlign: 'left',
                            ...
                            if (this.extraTag) {
                                let tagRadius = (tagSize + 10) / 2,
                                    tagHeight = bgObj.height - 190
                                let extraArr = [
                                    {
                                        type: 'roundFillRect',
                                        ...
                                        allInfoCallback({ drawArray }) {
                                            return new Promise((resolve, reject) => {
                                                if (price_show_width && price_market_width) {
                                                    const roundFillRect = drawArray.find(item => item.id === 'roundFillRect');
                                                    roundFillRect.dx = u24 + parseInt(price_show_width) + priceTagWidth * 2 + 8 + parseInt(price_market_width)
                                    ...
                                    },
                                    {
                                        ...
                                        infoCallBack() {
                                            if (price_show_width && price_market_width) {
                                                return {
                                                    dx: u24 + parseInt(price_show_width) + priceTagWidth * 2 + 8 + parseInt(price_market_width) + 15,
                                                }
                                            }
                                        },
                                    },
