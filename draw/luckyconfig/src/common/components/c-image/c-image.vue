...
export default {
    ...
    computed: {
        ...
        style_() {
            ...
            // #endif
            style.opacity = !this.loadStatus && this.showLoading && this.errSrc_ === this.imgUrl_ ? 0 : 1
            return style
        },
        parentStyle_() {
            const parentStyle = {
    ...
    methods: {
        getFullOssImg(url) {
            if (url.indexOf('data:image') !== -1 || url.indexOf('wxfile') !== -1 || url.indexOf('file') !== -1 || url.indexOf('_doc') !== -1) {
                ...
                let ossImg
                // #ifdef APP-PLUS-NVUE
                ossImg = (config ? (this.type === 'upload' ? config.baseOssImgUrl : config.ossImgUrl) : '') + url
                // #endif
                // #ifndef APP-PLUS-NVUE
                ossImg = (this.$config ? (this.type === 'upload' ? this.$config.baseOssImgUrl : this.$config.ossImgUrl) : '') + url
                // #endif
                return ossImg
            }
        },
