<template>
    <view>
		<c-colors class="c-page-head" :class="{ 'c-page-head__fixed': fixed_ }" :pro="pro_" :theme="theme_">
			<c-status-bar />
...
<script>
    ...
    export default {
        ...
        computed: {
			...
			gradientTitle_() {
                return String(this.gradientTitle) === 'false' ? false : true;
			},
            // 导航栏内部盒子的样式
            navbarInnerStyle_() {
            	let style = {
                    height: this.height_ + 'px',
					color: !this.gradientTitle_ ? this.color : !this.startColor && !this.endColor ? this.color : this.$c.colorToRgba(this.color, this.scrollHeight / 44)
                };
            ...
            titleStyle_() {
                 let titleSize =  this.titleSize / 2, fontWeight = 'bold';
                // #ifdef MP
            ...
            back_() {
                return String(this.back) === 'false' ? false : true;
            },
            fixed_() {
                return String(this.fixed) === 'false' ? false : true;
            },
			pro_() {
			...
			theme_() {
                ...
                let hex = this.scrollHeight > 44 ? 1 : ((this.scrollHeight < 10 ? '0.0' : '0.') + (this.scrollHeight * 2));
				if (!this.endColor || !this.startColor) {
				    let backgroundColor = this.startColor || this.endColor || 't';
					return [ `${backgroundColor}|${hex}` ]
				}else{
					let startColor = this.startColor.length > 1 ? this.$c.colorToRgba(this.startColor, hex) : this.startColor;
					let endColor = this.endColor.length > 1 ? this.$c.colorToRgba(this.endColor, hex) : this.endColor;
