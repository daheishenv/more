<template>
    <view>
        <c-popup mode="bottom" close v-model="selectFlag" radius="24rpx 24rpx 0 0">
            <view class="select-address-modal">
                <view class="c-modal__title">发送至</view>
                <scroll-view scroll-y class="c-row select-address__scroll ptb0 c-underline" >
                    <block v-if="pageList && pageList.length">
                        <c-radio-group v-model="selectIndex" v-for="(item, index) in pageList" :key="index" @change="changeAddress(item,index)" wrap>
                            <view class="c-col-12 ptb24 c-underline">
                                <c-radio :name="index">
                                    <c-address-item :config="item" edit="false"></c-address-item>
                                </c-radio>
                            </view>
                        </c-radio-group>
                    </block>
                    <c-no-data showImg text="暂无地址" v-else></c-no-data>
                </scroll-view>
                <view class="p24" >
                  <c-colors :theme="['t', '#fff']" :pro="['bgc', 'c']" radius="40">
                    <c-button type="inherit" @click="handleOther">{{mode === 'local' ? '选择其他地区': '新增地址'}}</c-button>
                  </c-colors>
                </view>
            </view>
        </c-popup>
        <!-- 新增地址 -->
        <c-edit-address v-model="selectOtherFlag" @change="changeAddress($event,'refresh')" />
        <!-- 选择地址 -->
        <c-select-city v-model="selectLocalFlag" @confirm="handleSelectCity" />
