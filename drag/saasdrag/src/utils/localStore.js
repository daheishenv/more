...
let storeKey = {
    ...
    CURRENT_PROJCET_INDEX: 'CURRENT_PROJCET_INDEX'
}
/**
 * 存储当前选中项目的索引
 * @param {Number} index 
 */
let cachesProjcetIndex = (index) => {
    _setLocalStore(storeKey.CURRENT_PROJCET_INDEX, index);
}
/**
 * 获取当前选中项目的索引 
 */
let getCachesProjcetIndex = () => {
    let result = _getLocalStore(storeKey.CURRENT_PROJCET_INDEX);
    if (result && result !== '') {
