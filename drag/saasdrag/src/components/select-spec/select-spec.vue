<template>
    <view>
        <c-popup v-model="modalFlag" mode="bottom" radius="24rpx 24rpx 0 0">
            <view class="select-rule-box">
                <view class="top-box flex">
                    <view class="flex-image">
                        <c-image size="180" radius="8" :src="curSku_.product_img"></c-image>
                    </view>
                    <view class="flex-1 pl24">
                        <c-colors :theme="[ priceColor ]">
                          <view class="price-com" :class="[tempCurSku.activity_type == 4 ? 'integral-price': '']">
							<c-image v-if="tempCurSku.activity_type == 4" :src="'integral/gold-coin.png'" size="32" class="integral-coin"></c-image>
                            {{ priceShow_ || '' }}
                            <view class="price-com__samll" v-if="priceShowUnit_ && saleSet_">{{priceShowUnit_}}</view>
                          </view>
                        </c-colors>
                ...
                <scroll-view scroll-y class="goods-rule-box" v-if="goods_sku_attr_value.length">
                    <view class="goods-rule-content">
                        <view v-for="(item, pIndex) in goods_sku_attr_value" :key="item.id">
                            <view class="goods-rule-title">{{item.attr_name}}</view>
                            <view class="goods-rule-list">
                                <view class="p12" v-for="(type, index) in item.goods_type_attr" :key="type.id" type="button">
									<c-colors :theme="[type.checked ? blockColor : '#333', type.checked ? `${blockColor}|0.06` : '#f5f5f5', type.checked ? blockColor : '#f5f5f5']" :pro="['c', 'bgc', 'bdc']" radius="8">
										<c-button height="68" plain v-if="type.attr_value" :class="{'select': type.checked}" @click="handleSelect(pIndex, index)">
                ...
                <view class="goods-rule-calc">
                    ...
                    <view class="calc-box">
                        <view class="flex-ajcenter" @click.stop.prevent>
                            <c-icons class="calc-btn" :class="{'calc-btn__disabled': minNum_}" @click="handleUpdateNum(false)" type="icon-jian" :color="minNum_ ? '#bbb' : '#333'"></c-icons>
                            <view class="flex-middle flex-1">
                                <text class="flex-middle__text d-din-bold">{{tempCurSku.num}}</text>
                            </view>
                            <c-icons class="calc-btn" :class="{'calc-btn__disabled': maxNum_}" @click="handleUpdateNum(true)" type="icon-jia" :color="maxNum_ ? '#bbb' : '#333'"></c-icons>
                        </view>
                    </view>
                </view>
				<view class="c-underline__top select-rule-btns c-row">
					<block v-if="action_ === 0">
						<view class="p12 flex-1" v-if="!activityInfo_.id">
							<c-colors :theme="joinTheme" :pro="['bgc', 'c']" :radius="radius_" type="button">
								<c-button :disabled="joinDisabled_" @click="handleSubmit('join')">
									{{joinText ? joinText : '加入购物车'}}
								</c-button>
							</c-colors>
						</view>
						<view class="p12 flex-1">
							<c-colors :theme="buyTheme" :pro="['bgc', 'c']" :radius="radius_" type="button">
								<c-button :theme="buyTheme" :pro="['bgc', 'c']" :disabled="buyDisabled_" @click="handleSubmit('buy')">
					...
					<block v-else-if="action_ === 2">
						<view class="p12 flex-1">
							<c-colors :theme="buyTheme" :pro="['bgc', 'c']" :radius="radius_" type="button">
								<c-button :disabled="joinDisabled_" @click="handleSubmit('buy')">
									{{buyText ? buyText : '立即购买'}}
								</c-button>
							</c-colors>
						</view>
...
<script>
    export default {
    	...
    	computed: {
			...
			buyDisabled_() {
				return String(this.buyDisabled) === 'false' ? false : true;
			},
			joinDisabled_() {
				return String(this.joinDisabled) === 'false' ? false : true;
			},
    		activityInfo_() {
			...
			buyMax_() {
				return this.activityInfo_.stint_num ? this.activityInfo_.stint_num : (this.activityInfo_.quantity_limit ? this.activityInfo_.quantity_limit : this.skuStock_);
			},
    		priceShowUnit_() {
    	...
    	methods: {
    		...
    		async handleSubmit(action) {
    			...
    			const { data: { token } } = await this.$http('wxuserinfo', null, {
    				source: 'catch'
    			});
    			if (!token) {
