<template>
    ...
    >
        <view class="ggc-subscribe c-content">
            <view v-if="mode === 'home'">
                <view class="ggc-subscribe__title">{{ title }}</view>
                <view class="ggc-subscribe__content pt16">服务内容：{{ content }}</view>
                <view class="ggc-subscribe__desc pt8">稍后客服将会联系您!</view>
            </view>
            <view v-else-if="mode === 'list'">
                <view class="ggc-subscribe__content">{{ content }}</view>
                <view class="ggc-subscribe__desc pt8">活动时间：{{ info.start_time }}-{{ info.end_time }}</view>
            </view>

            <text class="ggc-subscribe__input-title pt32">
                请确定手机号以便享受优惠
            </text>
            <view class="ggc-subscribe__input-box">
                <c-input
                    ref="checkTelRef"
                    v-model="mobile"
                    class="flex-1"
                ...
                >
                    <text
                        v-show="isEdit"
                        class="ggc-subscribe__input-operate"
                        @click="handleEdit"
                    >修改</text>
                </c-input>
            </view>
            <view v-if="mode === 'home'">
                <c-checkbox-group @change="handleCheckbox($event)">
                    <view class="ggc-subscribe__agreement pt16">
                        <c-checkbox
                            icon-size="28"
                            label-size="28"
                            name="checked"
                        ...
                        <text
                            ...
                            class="ggc-subscribe__agreement-text"
