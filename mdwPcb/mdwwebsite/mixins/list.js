export default {
    ...
    methods: {
        // 重置初始化数据
        async initPage(saveParams = true, refresh = false) {
            // 初始化所有变量
            this.page = 1
            this.busy = false
            this.finish = false
            ...
            await this.getCommonList(refresh)
        },
        // 上啦加载函数
        reachBottomFn() {
            this.getCommonList()
        ...
        async getCommonList(refresh) {
            ...
            try {
                ...
                let { data } = await this.$axios.$post(this.reqName, query)
