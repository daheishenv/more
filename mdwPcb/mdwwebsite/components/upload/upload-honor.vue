...
export default {
    ...
    methods: {
        ...
        handleInput(item) {
            const { id, src } = item
            if (id || src) {
                const emitChange = { detail: { value: item } }
                this.$emit('upload', emitChange)
            }
        },
        handleRemove({ id }) {
            this.$confirm('此操作将永久删除该荣誉资质, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                const emitChange = { detail: { value: { id } } }
        ...
        async uploadSuccess() {
            try {
                ...
                formData.set('file', this.fileInfo.raw)
